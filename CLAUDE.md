...
2. **每个页面的 UI 可视要素对比**（`PRD ↔ 当前实现` 对比）
3. **每个页面的控件矩阵**（控件级而非组件树级；至少覆盖筛选控件 / 表格列 / 按钮菜单 / 状态标签 / 弹窗入口 / 分页工具栏 / 空态提示）
4. **每个页面的字段对比**（`PRD vs 当前实现`）
5. **每个页面的 9 维度对比**（字段类项和控件矩阵不计入 9 维度）
6. **每个页面的验收点**（正向 / 空态 / 异常态 / 权限态；未核对则计入 blocker）
7. **差异清单 + Blockers**
...
- "页面看起来差不多"
- 任何没有页面级对比表格的模块级总结
- 建议决议只写几条笼统总结，未逐项对应差异清单中的每个 Dx/Bx
- 某些页面缺少控件矩阵 / 9 维度 / 验收点，但声称"已核对"
- 详细对比表里已标注「差异」的行未出现在差异清单中
- 对比表中出现「待核对」「需核对」等未决结论（文件存在但未读取源码就不能写对比表）
