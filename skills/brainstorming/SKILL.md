...
| 3.2 接口详细定义 | 非 CRUD 接口有请求体 DTO；每个接口列出请求参数和响应结构 |
| 4.2 关键字段 | 与前端类型设计**逐字段对齐**，**不得用省略号** |
| 4.3 DTO 设计 | 查询参数 DTO 覆盖前端 `queryParams` 所有字段；非标准操作有请求体 DTO |
| 5.1 表字段 | **完整列出**，不得用省略号 `...` |
| 6.1 方法清单 | 覆盖 3.1 中**每个接口** |
| 7.1 查询 SQL | **每个查询接口**都有 SQL 或伪 SQL |
| 7.2 写入 SQL | 关键写入操作有具体 SQL（多表事务写清多条 SQL） |
| 8 数据权限 | 不同角色的数据过滤方式已说明 |
| 11 前后端对齐 | 四张对齐表已填写（接口覆盖 + 请求参数 + 响应字段 + 枚举值） |
| 12 自检清单 | 模板 Section 12 的 12 项检查全部标记 ✅ 才可保存 |
