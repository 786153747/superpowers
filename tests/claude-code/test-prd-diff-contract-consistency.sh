...
grep -q 'PRD ↔ 当前实现' "$REPO_ROOT/skills/prd-diff-scan/SKILL.md"
echo "  [PASS] skill uses PRD vs current implementation comparison"
...
grep -q 'PRD ↔ 当前实现.*双方对比' "$REPO_ROOT/spec/frontend/vue/requirement-clarification.md"
echo "  [PASS] frontend spec uses dual comparison"

grep -q '字段类项.*不计入 9 维度' "$REPO_ROOT/CLAUDE.md"
...
grep -q 'PRD ↔ 当前实现.*双方对比' "$REPO_ROOT/CLAUDE.md"
echo "  [PASS] top-level CLAUDE uses dual comparison"
