...
| 15 | **原型字段覆盖**：对照当前 diff 文档（`diff_<commitid>.md`）中的 PRD 字段对比表，Section 6 类型设计的实体字段、Section 3.5 控件矩阵的筛选/表格/表单字段，必须与 PRD 要求的字段**逐项覆盖**——PRD 中有的字段不得遗漏，PRD 中没有的字段不得凭空新增（后端独有的审计字段如 createTime/delFlag 除外）。**控件类型不得降级**：PRD 中明确描述为「弹框选择」「下拉单选」「日期区间」等特殊控件类型时，控件矩阵必须体现对应类型和调用接口，不得改写为普通文本输入框。如有当前 diff 文档的差异决议明确「不做」的字段，标注跳过原因即可 | |
