...
>
> **禁止合并控件行**：每个控件独占一行，不得写 `C11~C20 | 数据列 | ...` 这样的合并行。
> 表格列也是控件——每个表格列（如"订单编号列""物料号列"）必须有独立的行和业务说明。
>
...
- **页面路由**：
- **后端菜单 component**：对应 `src/views/` 下的文件路径，不含 `.vue` 后缀。
  - 文件在 `views/order/myOrder.vue` → component = `order/myOrder`
  - 文件在 `views/order/myOrder/index.vue` → component = `order/myOrder/index`
- **按钮权限**：
...
> **API 契约真相来源规则**：
> - 若当前页面已有 UI 实现/原型实现，接口 path / method / 入参 / 出参必须从该页面的精确 UI 文件提取：页面 `.vue` + API 文件 + 类型文件 + 直接相关 mock 文件（如有）
> - `diff.md` / PRD 只能补充业务语义或已确认改动，不能在没有明确决议时覆盖 UI 已在使用的 API 契约
> - 每一行都必须填写「来源文件/证据」，不得写凭空推断的接口

| 场景 | 方法 | 路径 | 返回类型 | 来源文件/证据 |
| --- | --- | --- | --- | --- |
| 列表 | GET | `/.../list` | `TableDataInfo<T>` | `src/api/...` + `src/views/...` |
| 明细 | GET | `/.../{id}` | `AjaxResult<T>` | `src/api/...` + `src/views/...` |
| 新增 | POST | `/...` | `AjaxResult` | `src/api/...` + `src/views/...` |
| 修改 | PUT | `/...` | `AjaxResult` | `src/api/...` + `src/views/...` |
| 删除 | DELETE | `/.../{id}` | `AjaxResult` | `src/api/...` + `src/views/...` |
| 导出 | POST | `/.../export` | blob（由框架处理） | `src/api/...` + `src/views/...` |

### 调用约束（若依框架）

- 所有接口调用必须使用 `@/utils/request`，禁止自建 axios 实例
- 导出接口不得用 `request + responseType: blob` 手动处理下载，必须用 `proxy.download(url, params, filename)`
- API 文件统一放在 `src/api/<模块>/index.ts`，使用 `export function` 命名导出

## 6. 类型设计

> **字段来源规则**：
> - 若当前页面已有 UI 实现/原型实现，字段名、嵌套结构、可选性优先从精确 UI 文件提取
> - 每个字段或类型至少要能追溯到：页面文件 / API 文件 / 类型文件 / mock 文件 / 明确 diff 决议 之一
...
- 点击 → 调用 `proxy.download('{{module}}/{{resource}}/export', { ...queryParams }, '{{文件名}}.xlsx')`
- `proxy.download` 内置 loading、blob 下载、响应头解析，**禁止手动构造 blob 链接或使用 `request + responseType: blob`**
- 失败：框架自动提示错误，无需额外处理
...
| 15 | **原型字段覆盖**：对照当前 diff 文档（`diff.md`）中的 PRD 字段对比表，Section 6 类型设计的实体字段、Section 3.5 控件矩阵的筛选/表格/表单字段，必须与 PRD 要求的字段**逐项覆盖**——PRD 中有的字段不得遗漏，PRD 中没有的字段不得凭空新增（后端独有的审计字段如 createTime/delFlag 除外）。**控件类型不得降级**：PRD 中明确描述为「弹框选择」「下拉单选」「日期区间」等特殊控件类型时，控件矩阵必须体现对应类型和调用接口，不得改写为普通文本输入框。如有当前 diff 文档的差异决议明确「不做」的字段，标注跳过原因即可 | |
| 16 | **文档自包含**：本文档不包含任何指向其他页面 frontend-detail-design.md 的引用（如"同 myOrder 页面"）。每个 section 都有完整内容，开发者不需要打开其他页面文档即可完成本页面开发 | |
| 17 | **CXX 编号一致**：3.3 组合图、3.4 交互图、3.5 控件矩阵、Section 8 交互规则使用**相同的 CXX 编号**引用同一控件；控件矩阵表头包含完整 8 列（控件 ID / 控件名称 / 类型 / 业务说明 / 触发事件 / 效果描述 / 影响的控件 / 调用接口）；**禁止合并控件行**（如 `C11~C20 数据列`），每个控件独占一行 | |
| 18 | **8.5 空态与异常使用表格格式**：`场景 | 展示 | 可操作控件`，不得用散文替代 | |
| 19 | **查询参数完整性**：Section 6 查询参数类型必须覆盖**所有查询条件字段**（包括基本搜索和高级搜索），不得只列基本搜索字段而遗漏高级搜索字段 | |
| 20 | **API 契约证据**：Section 5 的每个接口、Section 6 的每类关键字段，都能追溯到精确 UI 文件（页面/API/类型/mock）或明确 diff 决议；不得出现仅凭 PRD 摘要脑补出的 path / 参数 / 返回字段 | |
```
